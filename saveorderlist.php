...
function bydetail($objPHPExcel,$orderno)
{
	...
	$sheet->setTitle(iconv('big5','utf-8','明細'));
	...
	while(list($pname,$pprice,$ppic,$uoption,$pqty,$unick,$content,$ordertime,$sname,$stel,$sfax,$spic,$saddr,$sintro)=mysql_fetch_row($data_rows))
	{
		if(i==0)
		{
			...
			$sheet->setCellValue('A8',iconv('big5','utf-8','編號'));
			$sheet->setCellValue('B8',iconv('big5','utf-8','品名'));
			$sheet->setCellValue('C8',iconv('big5','utf-8','訂購人'));
			$sheet->setCellValue('D8',iconv('big5','utf-8','選項'));
			$sheet->setCellValue('E8',iconv('big5','utf-8','說明'));
			$sheet->setCellValue('F8',iconv('big5','utf-8','單價'));
			$sheet->setCellValue('G8',iconv('big5','utf-8','數量'));
			$sheet->setCellValue('H8',iconv('big5','utf-8','小計'));
			$sheet->setCellValue('I8',iconv('big5','utf-8','訂購時間'));
			$sheet->setCellValue('J8',iconv('big5','utf-8','圖片'));
			//set font style
			$styleArray = array(
	...
	$sheet->setCellValue('F'.$k,iconv('big5','utf-8','合計:'));
...
function byproduct($objPHPExcel,$orderno)
{
	...
	$sheet=$objPHPExcel->getActiveSheet();
	$sheet->setTitle(iconv('big5','utf-8','商品'));
	$sheet->mergeCells('A1:F2'); 
	$sheet->mergeCells('A3:F3');
	$sheet->mergeCells('A4:F4');
	$sheet->mergeCells('A5:F5');
	...
	while(list($pname,$pprice,$ppic,$pqty,$remark,$sname,$stel,$sfax,$spic,$saddr,$sintro)=mysql_fetch_row($data_rows))
	{
		if(i==0)
		{
			...
			$sheet->setCellValue('A8',iconv('big5','utf-8','編號'));
			$sheet->setCellValue('B8',iconv('big5','utf-8','品名'));
			$sheet->setCellValue('C8',iconv('big5','utf-8','說明'));
			$sheet->setCellValue('D8',iconv('big5','utf-8','單價'));
			$sheet->setCellValue('E8',iconv('big5','utf-8','數量'));
			$sheet->setCellValue('F8',iconv('big5','utf-8','小計'));
	...
	$sheet->setCellValue('D'.$k,iconv('big5','utf-8','合計:'));
...
function byperson($objPHPExcel,$orderno)
{
	...
	$sheet=$objPHPExcel->getActiveSheet();
	$sheet->setTitle(iconv('big5','utf-8','訂購人'));
	...
	$strSql="SELECT unick,GROUP_CONCAT(pname,':NT$',L.pprice,'/',uoption,'".iconv('big5','utf-8','/ 數量:')."',pqty,'".iconv('big5','utf-8','/ 說明:')."',content),sum(pqty*L.pprice),sname,stel,sfax,spic,saddr,sintro FROM ttlisttb AS L INNER JOIN ttprodtb AS P INNER JOIN ttstoretb AS S ON S.sno=P.sno AND L.pno=P.pno WHERE ono=$orderno GROUP BY unick ORDER BY unick,ordertime desc";
	$data_rows=mysql_query($strSql,$link);
	$i=0;
	$k=9; //data from row 8
	...
	while(list($unick,$remark,$sum,$sname,$stel,$sfax,$spic,$saddr,$sintro)=mysql_fetch_row($data_rows))
	{
		if(i==0)
		{
			...
			$sheet->setCellValue('A6',$sintro);
			$sheet->setCellValue('A8',iconv('big5','utf-8','編號'));
			$sheet->setCellValue('B8',iconv('big5','utf-8','訂購人'));
			$sheet->setCellValue('C8',iconv('big5','utf-8','說明'));
			$sheet->setCellValue('D8',iconv('big5','utf-8','小計'));
	...
	$sheet->setCellValue('C'.$k,iconv('big5','utf-8','合計:'));
